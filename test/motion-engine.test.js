...
test("size pulse only shrinks the window, never past the base size", () => {
  const win = { id: 7, left: 400, top: 250, width: 700, height: 500 };
  const engine = new MotionEngine({ random: () => 0.5 });
  ...
  for (let index = 1; index <= 80; index += 1) {
    ...
    // 기본 크기를 절대 넘지 않는다.
    assert.ok(operation.width <= win.width);
    assert.ok(operation.height <= win.height);
    assert.ok(operation.width >= win.width * 0.5 - 1);
    assert.ok(operation.height >= win.height * 0.5 - 1);
    if (operation.width < win.width || operation.height < win.height) shrank = true;
  }
  assert.ok(shrank, "expected the window to shrink below the base size");
});
