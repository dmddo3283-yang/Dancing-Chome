...
test("popup starts capture from the current tab, not a desktop picker", async () => {
  ...
  assert.equal(popup.includes("tabCapture.getMediaStreamId"), true);
  assert.equal(popup.includes("src/capture/capture.html"), false);
});
